...
test_that(".rowDistinct reproduces per-row length(unique(.)) exactly", {
  ...
  for (L in c(1L, 3L, 9L)) {
    ...
    expect_identical(.rowDistinct(a, L), ref)
  }
  # single row / single column edges
  expect_identical(.rowDistinct(matrix(2L, 1L, 5L), 4L), 1L)
  expect_identical(.rowDistinct(matrix(1:4, 4L, 1L), 4L), rep(1L, 4L))
})
